...
<div class="tms-page-content" id="page-purchase">
    <div class="d-flex justify-content-between align-items-center mb-4">
            ...
            <div class="d-flex align-items-center gap-2">
                ...
                <h4 class="fw-bold mb-0 text-dark">
                    {{-- Add Item unit --}}
                    @if(request()->is('itemunit'))
                        <h2>Add Item Unit</h2>
                    @elseif(request()->is('item'))
                        <h2>Add item</h2>
                    @elseif (request()->is('trucks'))
                        <h2>Add Trucks</h2>
                    @elseif (request()->is('truckdocuments'))
                        <h2>Add Truck Documents</h2>
                    @elseif (request()->is('trucktypes'))
                        <h2>Add Truck Types</h2>
                    @elseif (request()->is('truckrenewaltypes'))
                        <h2>Add Truck Renewal Types</h2>
                    @elseif (request()->is('fasttag'))
                        <h2>Add FastTag</h2>
                    @elseif (request()->is('toll&dieselbudget'))
                        <h2>Add Toll & Diesel Budget</h2>
                    @elseif (request()->is('truckloanentry'))
                        <h2>Add Truck Loan Entry</h2>
                    @elseif (request()->is('truckEMIpayment'))
                        <h2>Add Truck EMI Payment</h2>
                    @elseif (request()->is('loanstatements'))
                        <h2>Add Loan Statements</h2>
                    @elseif (request()->is('dieselpurchase'))
                        <h2>Add Diesel Purchase</h2>
                    @elseif (request()->is('dieselstatements'))
                        <h2>Add Diesel Statements</h2>
                    @elseif (request()->is('truckmaintenance'))
                        <h2>Add Truck Maintenance</h2>
                    @elseif (request()->is('tripplanning'))
                        <h2>Add Trip Planning</h2>
                    @elseif (request()->is('lrtobill'))
                        <h2>Add LR to Bill</h2>
                    @elseif (request()->is('billlist'))
                        <h2>Add Bill List</h2>  
                    @elseif (request()->is('moneyreceipt'))
                        <h2>Add money receipt</h2>  
                    @elseif (request()->is('moneypaid'))
                        <h2>Add money paid</h2>
                    @elseif (request()->is('bankcash'))
                        <h2>Add bank cash</h2>
                    @elseif (request()->is('journalvoucher'))
                        <h2>Add journal voucher</h2>
                    @elseif (request()->is('fasttagrecharge'))
                        <h2>Add FastTag recharge</h2>  
                    @elseif (request()->is('dayattendance'))
                        <h2>Add day wise attendance</h2> 
                    @elseif (request()->is('truckstatments'))     
                        <h2>Add truck statements</h2>       
                    @endif
                </h4>
            </div>
    ...
    Add New Expense
                ...
                <a href="{{ route('itemunit.create') }}" class="btn btn-outline-primary btn-sm bg-white border">
                    <i class="fas fa-list me-1"></i> Create item unit
                </a>
