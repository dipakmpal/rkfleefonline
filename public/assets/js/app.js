...
document.addEventListener('DOMContentLoaded', function() {
    ...
    menuLinks.forEach(link => {
        link.addEventListener('click', function(e) {
            ...
            // Only use the SPA page-switching behavior for placeholder links.
            // Real URLs should navigate normally.
            if (targetPageId && !isRealLink) {
                e.preventDefault();
    ...
    // 6. Action Functions / Toast Notifications
    window.showToast = function(message, type = 'danger') {
        ...
        alertItem.className = `tms-alert-item`;
        alertItem.style.borderLeftColor = `var(--tms-${type})`;
        
        const iconClass = type === 'danger' ? 'fa-exclamation-circle' : 'fa-info-circle';
        
        alertItem.innerHTML = `
            <i class="fas ${iconClass} mt-1 text-${type}"></i>
            <div style="flex-grow: 1;">
                <div class="fw-bold small text-${type}">${type.toUpperCase()} ALERT</div>
                <div class="small text-muted" style="font-size: 0.75rem;">${message}</div>
            </div>
            <button type="button" class="btn-close" style="font-size: 0.6rem;" onclick="this.parentElement.remove()"></button>
        `;
        ...
        setTimeout(() => {
            alertItem.style.animation = 'slideInRight 0.3s reverse';
